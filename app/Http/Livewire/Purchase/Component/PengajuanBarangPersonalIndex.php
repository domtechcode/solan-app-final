...
class PengajuanBarangPersonalIndex extends Component
{
    ...
    {
        $dataPengajuanBarangSpk = PengajuanBarangSpk::where('status_id', '!=', 16)
            ->with(['status', 'workStepList', 'instruction', 'user'])
            ->orderBy('tgl_target_datang', 'asc')
            ->paginate($this->paginate);

        return view('livewire.purchase.component.pengajuan-barang-personal-index', ['pengajuanBarangSpk' => $dataPengajuanBarangSpk])
            ->extends('layouts.app')
            ->section('content')

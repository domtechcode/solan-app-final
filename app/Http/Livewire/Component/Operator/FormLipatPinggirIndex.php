...
class FormLipatPinggirIndex extends Component
{
    use WithFileUploads;
    ...
    public function mount($instructionId, $workStepId)
    {
        ...
        $dataSortir = FormLipatPinggir::where('instruction_id', $this->instructionCurrentId)
        ...
        $dataAnggotaCurrent = FormLipatPinggir::where('instruction_id', $this->instructionCurrentId)
            ->where('jenis_pekerjaan', $dataWorkStep->workStepList->name)
            ->get();
        if (isset($dataAnggotaCurrent)) {
            foreach ($dataAnggotaCurrent as $item) {
                $anggota = [
                    ...
                    'pekerjaan_per_anggota' => $item['pekerjaan_per_anggota'],
    ...
    {
        return view('livewire.component.operator.form-lipat-pinggir-index');
    }
    ...
    public function save()
    {
        $this->validate([
            ...
            'anggota.*.pekerjaan_per_anggota' => 'required',
        ...
        if ($currentStep->flag == 'Split' && $previousStep->state_task != 'Complete' && $previousStep->work_step_list_id != 2) {
            ...
            if (isset($this->anggota)) {
                $deleteFormFinishing = FormLipatPinggir::where('instruction_id', $this->instructionCurrentId)
                    ->where('jenis_pekerjaan', $currentStep->workStepList->name)
                    ->delete();

                foreach ($this->anggota as $dataAnggota) {
                    $createFormFinishing = FormLipatPinggir::create([
                        'instruction_id' => $this->instructionCurrentId,
                        ...
                        'pekerjaan_per_anggota' => $dataAnggota['pekerjaan_per_anggota'],
                    ]);
                }
            }

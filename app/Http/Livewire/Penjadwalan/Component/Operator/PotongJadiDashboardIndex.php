...
class PotongJadiDashboardIndex extends Component
{
    use WithPagination;
    protected $paginationTheme = 'bootstrap';
    protected $updatesQueryString = ['search'];

    public $paginatePotongJadi = 10;
    public $searchPotongJadi = '';
    ...
    public function updatingSearchPotongJadi()
    {
        $this->resetPage();
    }
    ...
    {
        $this->searchPotongJadi = request()->query('search', $this->searchPotongJadi);
    }
    ...
    {
        $dataPotongJadi = WorkStep::where('work_step_list_id', 9)
            ...
            ->select('work_steps.*')
            ->with(['status', 'job', 'workStepList', 'instruction', 'user', 'machine'])
            ->orderBy('instructions.shipping_date', 'asc')
            ->paginate($this->paginatePotongJadi);

        $instructionsByUserPotongJadi = $dataPotongJadi->groupBy(function ($item) {
            return $item->user->name; // Ubah nama kolom pengguna sesuai dengan struktur tabel Anda
        });

        return view('livewire.penjadwalan.component.operator.potong-jadi-dashboard-index', ['instructionsByUserPotongJadi' => $instructionsByUserPotongJadi, 'instructionsPotongJadi' => $dataPotongJadi])
            ->extends('layouts.app')
            ->section('content')
            ->layoutData(['title' => 'Dashboard']);
    }

    public function modalInstructionDetailsPotongJadi($instructionId)
    {
    ...
    public function modalInstructionDetailsGroupPotongJadi($groupId)
    {

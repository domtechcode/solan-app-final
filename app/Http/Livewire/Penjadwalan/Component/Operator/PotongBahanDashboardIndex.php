...
class PotongBahanDashboardIndex extends Component
{
    use WithPagination;
    protected $paginationTheme = 'bootstrap';
    protected $updatesQueryString = ['search'];

    public $paginatePotongBahan = 10;
    public $searchPotongBahan = '';
    ...
    public function updatingSearchPotongBahan()
    {
        $this->resetPage();
    }
    ...
    {
        $this->searchPotongBahan = request()->query('search', $this->searchPotongBahan);
    }
    ...
    {
        $dataPotongBahan = WorkStep::where('work_step_list_id', 8)
            ->where('state_task', 'Running')
            ->whereIn('status_task', ['Pending Approved', 'Process', 'Reject Requirements'])
            ->where('spk_status', 'Running')
            ->whereHas('instruction', function ($query) {
                $searchTerms = '%' . $this->searchPotongBahan . '%';
                $query
            ...
            ->paginate($this->paginatePotongBahan);

        $instructionsByUserPotongBahan = $dataPotongBahan->groupBy(function ($item) {
            return $item->user->name; // Ubah nama kolom pengguna sesuai dengan struktur tabel Anda
        });

        return view('livewire.penjadwalan.component.operator.potong-bahan-dashboard-index', ['instructionsByUserPotongBahan' => $instructionsByUserPotongBahan, 'instructionsPotongBahan' => $dataPotongBahan])
            ->extends('layouts.app')
            ->section('content')
            ->layoutData(['title' => 'Dashboard']);
    }

    public function modalInstructionDetailsPotongBahan($instructionId)
    {
    ...
    public function modalInstructionDetailsGroupPotongBahan($groupId)
    {

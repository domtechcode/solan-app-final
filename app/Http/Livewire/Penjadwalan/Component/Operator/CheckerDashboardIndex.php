...
class CheckerDashboardIndex extends Component
{
    use WithPagination;
    protected $paginationTheme = 'bootstrap';
    protected $updatesQueryString = ['search'];

    public $paginateChecker = 10;
    ...
    public function updatingSearchChecker()
    {
        $this->resetPage();
    }
    ...
    {
        $this->searchChecker = request()->query('search', $this->searchChecker);
    }
    ...
    public function render()
    {
        $dataChecker = WorkStep::where('work_step_list_id', 37)
            ...
            ->paginate($this->paginateChecker);

        $instructionsByUserChecker = $dataChecker->groupBy(function ($item) {
            return $item->user->name; // Ubah nama kolom pengguna sesuai dengan struktur tabel Anda
        });

        return view('livewire.penjadwalan.component.operator.checker-dashboard-index', ['instructionsByUserChecker' => $instructionsByUserChecker , 'instructionsChecker' => $dataChecker])
            ->extends('layouts.app')
            ->section('content')
            ->layoutData(['title' => 'Dashboard']);
    }

    public function modalInstructionDetailsChecker($instructionId)
    {
    ...
    public function modalInstructionDetailsGroupChecker($groupId)
    {

...
class PondDashboardIndex extends Component
{
    use WithPagination;
    protected $paginationTheme = 'bootstrap';
    protected $updatesQueryString = ['search'];

    public $paginatePond = 10;
    public $searchPond = '';
    ...
    public function updatingSearchPond()
    {
        $this->resetPage();
    }
    ...
    {
        $this->searchPond = request()->query('search', $this->searchPond);
    }
    ...
    public function render()
    {
        $dataPond = WorkStep::where('work_step_list_id', 24)
            ...
            ->paginate($this->paginatePond);

        $instructionsByUserPond = $dataPond->groupBy(function ($item) {
            return $item->user->name; // Ubah nama kolom pengguna sesuai dengan struktur tabel Anda
        });

        return view('livewire.penjadwalan.component.operator.pond-dashboard-index', ['instructionsByUserPond' => $instructionsByUserPond, 'instructionsPond' => $dataPond])
            ->extends('layouts.app')
            ->section('content')
            ->layoutData(['title' => 'Dashboard']);
    }

    public function modalInstructionDetailsPond($instructionId)
    {
    ...
    public function modalInstructionDetailsGroupPond($groupId)
    {

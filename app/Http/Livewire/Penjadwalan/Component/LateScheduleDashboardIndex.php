...
class LateScheduleDashboardIndex extends Component
{
    use WithPagination;
    protected $paginationTheme = 'bootstrap';
    protected $updatesQueryString = ['search'];

    public $paginateLateSchedule = 10;
    public $searchLateSchedule = '';
    ...
    public function updatingSearchLateSchedule()
    {
        $this->resetPage();
    }
    ...
    {
        $this->searchLateSchedule = request()->query('search', $this->searchLateSchedule);
    }
    ...
    public function render()
    {
        ...
        $dataLateSchedule = WorkStep::where('work_step_list_id', 2)
            ->where('schedule_state', 'Late By Schedule')
            ->where(function ($query) {
                $searchTerms = '%' . $this->searchLateSchedule . '%';
                $query
            ...
            ->paginate($this->paginateLateSchedule);

        return view('livewire.penjadwalan.component.late-schedule-dashboard-index', ['instructionsLateSchedule' => $dataLateSchedule])
            ->extends('layouts.app')
            ->section('content')
            ->layoutData(['title' => 'Dashboard']);
    }

    public function reschedulelateschedule()
    {
        $this->validate(
            [
        ...
        $this->emit('indexRender');
        $this->dispatchBrowserEvent('close-modal-late-schedule');
    }

    public function modalInstructionDetailsLateSchedule($instructionId)
    {
        $this->workSteps = [];
        $this->pengajuanBarang = [];
    ...
    public function modalInstructionDetailsGroupLateSchedule($groupId)
    {
        $this->workSteps = [];

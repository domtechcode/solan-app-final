                    ...
                    @if (isset($dataMaklun))
                        {{-- @if ($dataMaklun->pekerjaan == 'Purchase')
                            ...
                            @elseif(in_array($dataMaklun->status, ['Approve Accounting', 'Approve RAB']))
                                <button class="btn btn-success" wire:click="completeMaklun({{ $dataMaklun->id }})" wire:key="completeMaklun({{ $dataMaklun->id }})">Complete</button>
                                <button class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            @elseif($dataMaklun->status == 12)
                                <button class="btn btn-info" wire:click="ajukanAccountingMaklun({{ $dataMaklun->id }})" wire:key="ajukanAccountingMaklun({{ $dataMaklun->id }})">Ajukan <i class="fe fe-arrow-right"></i> Accounting</button>
                                <button class="btn btn-info" wire:click="ajukanRabMaklun({{ $dataMaklun->id }})" wire:key="ajukanRabMaklun({{ $dataMaklun->id }})">Ajukan <i class="fe fe-arrow-right"></i> Rab</button>

    ...
    <div class="row">
        <div class="col">
            <select id="" name="" class="form-control form-select w-auto"
                wire:model="paginatePengajuanProcessMaklunSpk">
        ...
        <div class="col d-flex justify-content-end">
            <input type="text" class="form-control w-auto" placeholder="Search"
                wire:model="searchPengajuanProcessMaklunSpk">
        </div>
    </div>
    <div class="row mt-3">
        <div class="col-md-12">
                    ...
                    <tbody>
                        @forelse ($pengajuanProcessMaklunSpk as $key => $itemPengajuanProcessMaklunSpk)
                            <tr wire:key="{{ $itemPengajuanProcessMaklunSpk->id }}">
                                ...
                                <td>
                                    {{ $itemPengajuanProcessMaklunSpk->bentuk_maklun }}
                                </td>
                                <td>
                                    {{ $itemPengajuanProcessMaklunSpk->rekanan }}
                                </td>
                                <td>
                                    {{ $itemPengajuanProcessMaklunSpk->tgl_keluar }}
                                </td>
                                <td>
                                    {{ $itemPengajuanProcessMaklunSpk->qty_keluar }}
                                </td>
                                <td>
                                    {{ $itemPengajuanProcessMaklunSpk->satuan_keluar }}
                                </td>
                                @if ($itemPengajuanProcessMaklunSpk->status == 'Pengajuan Purchase')
                                    <td>
                                        <span
                                            class="badge bg-secondary rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->status }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge bg-secondary rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->pekerjaan }}</span>
                                    </td>
                                @elseif(in_array($itemPengajuanProcessMaklunSpk->status, ['Pengajuan Accounting', 'Pengajuan RAB']))
                                    <td>
                                        <span
                                            class="badge bg-info rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->status }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge bg-info rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->pekerjaan }}</span>
                                    </td>
                                @elseif(in_array($itemPengajuanProcessMaklunSpk->status, ['Reject Accounting', 'Reject RAB']))
                                    <td>
                                        <span
                                            class="badge bg-primary rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->status }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge bg-primary rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->pekerjaan }}</span>
                                    </td>
                                @else
                                    <td>
                                        <span
                                            class="badge bg-success rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->status }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge bg-success rounded-pill text-white p-2 px-3">{{ $itemPengajuanProcessMaklunSpk->pekerjaan }}</span>
                                    </td>
                                @endif
                                <td>
                                    <div class="btn-list">
                                        <button class="btn btn-icon btn-sm btn-dark" data-bs-toggle="modal"
                                            data-bs-target="#modalPengajuanProcessMaklunSpk"
                                            wire:click="modalPengajuanProcessMaklunSpk({{ $itemPengajuanProcessMaklunSpk->id }}, {{ $itemPengajuanProcessMaklunSpk->instruction_id }})"
                                            wire:key="modalPengajuanProcessMaklunSpk({{ $itemPengajuanProcessMaklunSpk->id }}, {{ $itemPengajuanProcessMaklunSpk->instruction_id }})"><i
        ...
        <div class="col d-flex justify-content-end mt-3">
            {{ $pengajuanProcessMaklunSpk->links() }}
        </div>
    </div>

    <!-- Modal General-->
    <div wire:ignore.self class="modal fade" id="modalPengajuanProcessMaklunSpk" tabindex="-1" role="dialog">
